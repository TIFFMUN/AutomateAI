...
async def send_otp(request: SendOTPRequest):
    """Send OTP code to user's email or phone"""
    try:
        otp_code, success = otp_service.send_otp(
            request.user_id, 
            request.contact, 
        ...
        if success:
            ...
            return {
                "success": False,
                "message": f"Failed to send OTP to {request.contact}"
            }
...
async def validate_otp(request: ValidateOTPRequest):
    """Validate OTP code"""
    try:
        is_valid, message = otp_service.validate_otp(
            request.user_id, 
            request.otp_code
        )
...
@app.get("/api/otp/info/{user_id}")
async def get_otp_info(user_id: str):
    """Get OTP information for a user"""
    try:
        otp_info = otp_service.get_otp_info(user_id)
        ...
        return {
            "success": False,
            "message": f"Error getting OTP info: {str(e)}"
        }
